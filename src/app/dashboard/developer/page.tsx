          ...
          <div className="max-w-6xl mx-auto space-y-12">
            ...
            <section className={`flex flex-col md:flex-row justify-between items-end gap-6 border-b border-slate-200/50 pb-8 animate-ui-slide-down ${isRtl ? 'md:flex-row-reverse' : ''}`}>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
                <Link 
                  href="/jobs"
                  className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-cyan-500 text-slate-900 font-extrabold text-sm hover:animate-btn-bounce shadow-lg shadow-cyan-500/20 transition-all active:scale-95 text-center flex items-center justify-center gap-2 group relative overflow-hidden"
                >
            ...
            {loading ? (
              ...
              <div className="text-center py-20 space-y-6 bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden animate-ui-scale-up" style={{ animationDelay: '150ms', animationFillMode: 'both' }}>
                 ...
                 <Link href="/jobs" className="inline-block px-10 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full font-black text-xs uppercase tracking-widest hover:animate-btn-bounce transition-all shadow-xl">{isRtl ? 'قدم عرضك الأول' : 'Deploy First Bid'}</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                {proposals.map((prop, index) => (
                  <div key={prop.id} 
                    className="group relative p-8 rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 transition-all duration-700 hover:-translate-y-3 hover:shadow-2xl overflow-hidden hover:animate-card-glow animate-ui-slide-down"
